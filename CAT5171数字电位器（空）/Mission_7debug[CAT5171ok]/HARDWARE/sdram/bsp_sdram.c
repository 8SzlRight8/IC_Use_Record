...
void SDRAM_Init(void)
{
 ...
 /* SDCLK： 90 Mhz (HCLK/2 :180Mhz/2) 1个时钟周期Tsdclk =1/90MHz=11.11ns*/
  ...
  /* TRCD: 15ns => 2x11.11ns ，行地址命令之后 和 发送列地址命令之间的延时 */
...
/* FMC SDRAM 控制配置 */
  ...
  /* SDCLK时钟分频因子，SDCLK = HCLK/SDCLOCK_PERIOD    2分频—>90M*/	
...
uint8_t SDRAM_Test(void)
{
  ...
  /* 把SDRAM数据全部重置为0 ，W9825G6K是以8位为单位的 */
...
{		
  ...
  */
  RCC_AHB1PeriphClockCmd(FMC_RAM_CS_GPIO_CLK  | FMC_RAM_WE_GPIO_CLK | FMC_RAM_OE_GPIO_CLK, ENABLE);                      
	...
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;       //配置为复用功能
...
void WS_YM_FUN(void)
{
  ...
  p.FSMC_DataSetupTime = 5;                   //数据建立时间
  p.FSMC_BusTurnAroundDuration = 2;           //总线恢复时间
  p.FSMC_CLKDivision = 2;                     //时钟分割系数
  p.FSMC_DataLatency = 2;                     //数据延时
  p.FSMC_AccessMode = FSMC_AccessMode_A;      //异步突发访问模式
  */
  ///*-- FMC Configuration ------------------------------------------------------
  /*
  //************AD 可以USB 不行*************************
  FMC_NORSRAMTimingInitStructure.FMC_AddressSetupTime = 15;//6;//3;                //地址建立时间
  FMC_NORSRAMTimingInitStructure.FMC_AddressHoldTime = 15;//6;//0;                 //地址保持时间
  FMC_NORSRAMTimingInitStructure.FMC_DataSetupTime = 200;//3;                   //数据建立时间
  FMC_NORSRAMTimingInitStructure.FMC_BusTurnAroundDuration = 0;//6;//时钟分割系数
  FMC_NORSRAMTimingInitStructure.FMC_CLKDivision = 14;//6;                     //时钟分割系数
  FMC_NORSRAMTimingInitStructure.FMC_DataLatency = 14;//6;//0;                     //数据延时
  FMC_NORSRAMTimingInitStructure.FMC_AccessMode = FMC_AccessMode_A;      //异步突发访问模式
  
  /*****************usb 可以  AD 不行************************
  FMC_NORSRAMTimingInitStructure.FMC_AddressSetupTime = 6;//3;                //地址建立时间
  FMC_NORSRAMTimingInitStructure.FMC_AddressHoldTime = 6;//0;                 //地址保持时间
  FMC_NORSRAMTimingInitStructure.FMC_DataSetupTime = 3;                   //数据建立时间
  FMC_NORSRAMTimingInitStructure.FMC_BusTurnAroundDuration = 6;//时钟分割系数
  FMC_NORSRAMTimingInitStructure.FMC_CLKDivision = 6;                     //时钟分割系数
  FMC_NORSRAMTimingInitStructure.FMC_DataLatency = 6;//0;                     //数据延时
  FMC_NORSRAMTimingInitStructure.FMC_AccessMode = FMC_AccessMode_A;      //异步突发访问模式
  */
  
  FMC_NORSRAMTimingInitStructure.FMC_AddressSetupTime = 15;       //地址建立时间
  FMC_NORSRAMTimingInitStructure.FMC_AddressHoldTime = 15;        //地址保持时间
  FMC_NORSRAMTimingInitStructure.FMC_DataSetupTime = 20;//5;//3;  //数据建立时间 大于10则AD采样数据稳定
  FMC_NORSRAMTimingInitStructure.FMC_BusTurnAroundDuration = 0;   //总线恢复时间
  FMC_NORSRAMTimingInitStructure.FMC_CLKDivision = 1;             //时钟分频因子
  FMC_NORSRAMTimingInitStructure.FMC_DataLatency = 1;             //数据分频因子
  FMC_NORSRAMTimingInitStructure.FMC_AccessMode = FMC_AccessMode_A;      //异步突发访问模式
