...
void SPI_FLASH_Init(void)
{
  ...
  SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;													//空闲时电平状态
  SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;												//偶数边缘
  SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;														//软件控制，所以引脚得设置成推挽输出 
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;	//频率2分频
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;									//MSB高位传输
  SPI_InitStructure.SPI_CRCPolynomial = 7;														// = 0就好，不需要使用CRC，随便写一个数就好
...
/**
* @brief  擦除FLASH块区:32KB
  * @param  SectorAddr：要擦除的扇区地址
  * @retval 无
  */
void SPI_FLASH_SBlockErase(u32 SectorAddr)
{
  /* 发送FLASH写使能命令 */
  SPI_FLASH_WriteEnable();
  SPI_FLASH_WaitForWriteEnd();
  ...
  SPI_FLASH_CS_LOW();
  /* 发送扇区擦除指令*/
  SPI_FLASH_SendByte(W25X_SBlockErase);
  /*发送擦除扇区地址的高8位*/
  SPI_FLASH_SendByte((SectorAddr & 0xFF000000) >> 24);
  /*发送擦除扇区地址的中前8位*/
  SPI_FLASH_SendByte((SectorAddr & 0xFF0000) >> 16);
 ...
 /**
* @brief  擦除FLASH块区:64KB
  * @param  SectorAddr：要擦除的扇区地址
  * @retval 无
  */
void SPI_FLASH_BBlockErase(u32 SectorAddr)
{
  /* 发送FLASH写使能命令 */
  SPI_FLASH_WriteEnable();
  SPI_FLASH_WaitForWriteEnd();
  ...
  SPI_FLASH_CS_LOW();
  /* 发送扇区擦除指令*/
  SPI_FLASH_SendByte(W25X_BBlockErase);
  /*发送擦除扇区地址的高8位*/
  SPI_FLASH_SendByte((SectorAddr & 0xFF000000) >> 24);
  /*发送擦除扇区地址的中前8位*/
  SPI_FLASH_SendByte((SectorAddr & 0xFF0000) >> 16);
...
/**
  * @brief  对FLASH写入数据，调用本函数写入数据前需要先擦除扇区
  * @param	pBuffer，要写入数据的指针
  * @param  Block_i，按一整块写入数据
  * @param  NumByteToWrite，写入数据长度
  * @retval 无
  */
void SPI_FLASH_BlockBufferWrite(u8* pBuffer, u32 Block_i, u32 Offset ,u32 NumByteToWrite)
{
	u32 WriteAddr = Block_i * SPI_FLASH_BlockSize + Offset;
  u8 NumOfPage = 0, NumOfSingle = 0, Addr = 0;
...
* @param   	Block_i:读取第几块数据，一块32KB
  * @param   NumByteToRead，读取数据长度
  * @retval 无
  */
void SPI_FLASH_BlockBufferRead(u8* pBuffer, u32 Block_i, u32 Offset, u32 NumByteToRead)
{
	u32 ReadAddr = Block_i * SPI_FLASH_BlockSize + Offset;
  /* 选择FLASH: CS低电平 */
  SPI_FLASH_CS_LOW();
